...
// ─── Diary ───
const DIARY_API = "http://119.91.115.102:9801";
  ...
  const fetchEntries = async () => {
    ...
    try {
      const [listRes, statsRes] = await Promise.all([
        fetch(`${DIARY_API}/diary/list?limit=50&show_locked=true`).then(r => r.json()),
        fetch(`${DIARY_API}/diary/mood_stats`).then(r => r.json()).catch(() => null),
      ]);
      setEntries(listRes);
  ...
  return (
    <div className="page">
      <div className="panel page-main">
        ...
        <div className="page-subtitle">{`锁着的只有${(window.__petConfig||{}).charName||'Pety'}能解锁 · 标题始终可见`}</div>
        {loading ? (
        ...
        {detail ? (
          ...
          <div className="surface" style={{ marginTop: 8 }}>
            <div style={{ fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.7 }}>
              · 点击左侧日记查看详情<br/>
              · 标题和心情始终可见<br/>
              {`· 锁着的内容只有${(window.__petConfig||{}).charName||'Pety'}能解锁`}
            </div>
          </div>
        )}
      </div>
...
const SettingsPage = ({ tweaks }) => {
  ...
  const [charName, setCharName] = React.useState("Pety");
  ...
  React.useEffect(() => {
    ...
    if (window.electronAPI && window.electronAPI.getApiConfig) {
      window.electronAPI.getApiConfig().then(cfg => {
        ...
        setCharName(cfg.charName || "Pety");
        setUserName(cfg.userName || "User");
          ...
          <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => {
              ...
              if (window.electronAPI && window.electronAPI.ttsGenerate) {
                window.electronAPI.ttsGenerate("你好，我是" + ((window.__petConfig||{}).charName||'Pety')).then(r => {
