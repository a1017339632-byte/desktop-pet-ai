...
const DIARY_URL = process.env.DIARY_URL || "";
const SCREENTIME_URL = process.env.SCREENTIME_URL || "";
...
function connectRelay() {
  ...
  relayWs.on("open", () => {
    relayWs.send(JSON.stringify({ role: "plugin", charName: process.env.CHAR_NAME || "", userName: process.env.USER_NAME || "" }));
...
function handleRelayMessage(data) {
  ...
  if (type === "call-result" && callResultHandler) {
...
async function main() {
  ...
  mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      {
        name: "call",
        description: "Call the user (voice call). Pet shows incoming call UI, 30s timeout. Result: accepted/declined/missed/busy",
        inputSchema: {
          type: "object",
          properties: {},
        },
  ...
  mcp.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "write_diary") {
      if (!DIARY_URL) {
        return { content: [{ type: "text", text: "日记功能未配置: 请设置 DIARY_URL 环境变量" }], isError: true };
      }
    ...
    if (name === "get_activity") {
      ...
      try {
        ...
        const relayHttp = RELAY_URL.replace(/^ws:/, "http:").replace(/^wss:/, "https:");
        const screentimeBase = SCREENTIME_URL || relayHttp.replace(/:8765/, ":9527");
        const [supervisor, screentime] = await Promise.all([
          fetchJson(relayHttp + "/api/supervisor"),
          fetchJson(screentimeBase + "/api/screentime/recent"),
        ]);
        if (supervisor) {
    ...
    if (name === "call") {
      const ok = await sendToRelay({ type: "call" });
      if (!ok) {
        return { content: [{ type: "text", text: "Pet app not online, cannot call" }], isError: true };
      }
      return new Promise((resolve) => {
        const timeout = setTimeout(() => {
          resolve({ content: [{ type: "text", text: "📞 未接听（超时）" }] });
        }, 35000);
        const handler = (data) => {
          if (data.type === "call-result") {
            clearTimeout(timeout);
            if (callResultHandler === handler) callResultHandler = null;
            const map = { accepted: "📞 Call accepted!", declined: "📞 Call declined", missed: "📞 Missed", busy: "📞 Busy (call in progress)", offline: "📞 Pet app not running" };
            resolve({ content: [{ type: "text", text: map[data.result] || `📞 ${data.result}` }] });
          }
        };
        callResultHandler = handler;
      });
    }
