...
function getVoiceConfig() {
  // Try multiple candidate paths for voice_config.json (portable exe may use a temp userData)
  const candidates = [];
  try { candidates.push(path.join(app.getPath('userData'), 'voice_config.json')); } catch (e) {}
  // exe directory (portable deployment)
  candidates.push(path.join(path.dirname(process.execPath), 'voice_config.json'));
  // app source directory
  candidates.push(path.join(__dirname, 'voice_config.json'));

  for (const cfgPath of candidates) {
    try {
      if (fs.existsSync(cfgPath)) {
        const cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
        console.log(`[voice-config] loaded from: ${cfgPath}`);
        return cfg;
      }
    } catch (e) {
      console.warn(`[voice-config] failed to read ${cfgPath}: ${e.message}`);
    }
  }
  console.warn(`[voice-config] no voice_config.json found in any candidate path:`);
...
function startVoiceServer() {
  ...
  console.log(`[voice] config: stt_engine=${cfg.stt_engine || 'unset'}, tencent_id=${cfg.tencent_secret_id ? 'set' : 'unset'}, minimax=${cfg.minimax_api_key ? 'set' : 'unset'}`);

  // Warn about missing API keys
  const sttEngine = cfg.stt_engine || 'tencent';
  if (sttEngine === 'tencent' && (!cfg.tencent_secret_id || !cfg.tencent_secret_key)) {
    console.warn('[voice] WARNING: STT engine is tencent but tencent_secret_id/key not configured. Will fall back to whisper.');
    broadcastVoiceStatus('loading', '腾讯云密钥未配置，将使用Whisper本地识别');
  }
  if (!cfg.minimax_api_key) {
    console.warn('[voice] WARNING: minimax_api_key not configured. TTS (语音合成) will be unavailable.');
    broadcastVoiceStatus('loading', 'MiniMax API Key未配置，TTS语音合成不可用');
  }
  ...
  voiceServerProcess.stdout.on('data', (d) => {
    ...
    else if (line.includes('Tencent SDK import failed')) broadcastVoiceStatus('error', '腾讯云SDK导入失败，使用Whisper');
    else if (line.includes('Tencent ASR failed')) broadcastVoiceStatus('loading', '腾讯云识别失败，已切换Whisper');
    else if (line.includes('Cannot bind port')) broadcastVoiceStatus('error', '端口9800被占用，可能有另一个语音服务在运行');
  });
...
function createPanelWindow() {
  ...
  panelWin.webContents.on('did-finish-load', () => {
    // Inject config for renderer-side URL access
    const cfg = JSON.stringify({ relayUrl: getRelayUrl(), diaryUrl: getDiaryUrl() });
    panelWin.webContents.executeJavaScript(`window.__PETY_CONFIG__ = ${cfg};`).catch(() => {});
    panelWin.webContents.send('relay-message', lastRelayStatus);
    messageHistory.forEach(msg => {
      panelWin.webContents.send('relay-message', msg);
    });
    // Restore voice mode state so panel syncs with float/main
    if (voiceModeActive) {
      panelWin.webContents.send('relay-message', { type: 'voice-mode', active: true });
    }
  });
...
ipcMain.on('relay-send', (event, data) => {
  ...
  if (apiConfig.mode === 'api') {
    ...
    if (data.type === 'poke') {
      const pokeText = `${apiConfig.userName || 'User'}戳了${apiConfig.charName || 'AI'}`;
      addLocalChat('user', pokeText, null, null, true);
      ipcMain.emit('api-chat', event, { text: pokeText, isEvent: true });
      return;
    }
    if (data.type === 'pet') {
      const petText = `${apiConfig.userName || 'User'}摸了${apiConfig.charName || 'AI'}`;
...
async function autoTtsForMessage(text) {
  ...
  try {
    ...
    if (!apiKey) {
      console.warn('[tts] minimax_api_key is empty, skipping TTS. Please configure in Settings.');
      return;
    }
...
ipcMain.handle('tts-generate', async (_, text) => {
  ...
  const apiKey = cfg.minimax_api_key || '';
  if (!apiKey) {
    return { success: false, error: 'MiniMax API Key未配置，请在设置中填写' };
  }
  const voiceId = cfg.tts_voice_id || 'moss_audio_fd2620f9-bef3-11f0-8647-a697af11f3d9';
  const body = JSON.stringify({
    model: 'speech-2.8-hd', text, stream: false,
...
// ===== API Mode (直连LLM) =====
let API_CONFIG_PATH = '';
let apiConfig = { mode: 'plugin', provider: 'openai', apiKey: '', baseUrl: '', model: '', charName: '', userName: '', systemPrompt: '', memoryTopN: 3, memoryRecentN: 5, memoryTodoN: 1, maxContextTokens: 20000, historyPreloadN: 10, relayUrl: 'ws://localhost:8765', diaryUrl: '' };
...
function loadApiConfig() {
  const candidates = [
    API_CONFIG_PATH,
    path.join(path.dirname(process.execPath), 'api_config.json'),
    path.join(__dirname, 'api_config.json'),
  ].filter(Boolean);
  for (const cfgPath of candidates) {
    try {
      if (fs.existsSync(cfgPath)) {
        ...
        console.log(`[config] api_config loaded from: ${cfgPath}`);
        return;
      }
    } catch (e) {}
  }
  console.warn('[config] api_config.json not found in:', candidates);
}
...
function resolveTemplate(text) {
  return text.replace(/\{\{char\}\}/g, apiConfig.charName || 'AI').replace(/\{\{user\}\}/g, apiConfig.userName || 'User');
}
      ...
      for (const tool of tools) {
        if (tool === 'screentime') {
          ...
          const callResult = await new Promise((resolve) => {
            handleIncomingCall({ caller: apiConfig.charName || 'AI' });
            if (activeCall) {
...
function handleIncomingCall(data) {
  ...
  callCallerName = apiConfig.charName || data.caller || 'AI';
...
app.whenReady().then(() => {
  ...
  console.log(`[app] exe path: ${process.execPath}`);
  CHAT_DB_PATH = path.join(userDataPath, 'chat_history.json');
  VOICE_DIR = path.join(userDataPath, 'voice');
  API_CONFIG_PATH = path.join(userDataPath, 'api_config.json');
  MEMORY_DB_PATH = path.join(userDataPath, 'pet_memories.json');
  loadLocalChat();
  loadApiConfig();
  loadMemoryConfig();
  loadMemories();
  ...
  tray.setToolTip(`${apiConfig.charName || 'Pety'} · Desktop Pet`);
